	...
	namespace fat
	{
		...
		void Fat32Inode::read_content( void * buf, uint64 read_len, uint64 offset, bool async_read )
		{
			if ( _dir_type == fat32nod_folder )
				printfYellow( "Fat32DirEntry: read folder entry content will get raw data." );
			...
			{
				printfYellow( "read offset beyond file size. no data will be read." );
				return;
			}
		...
		{
			if ( _dev_name.empty() )
				printfRed( "This is not a valid device file" );
			return dev::k_devm.search_device( _dev_name.c_str() );
		}
		...
		Inode * Fat32Inode::lookup( eastl::string dirname )
		{
			...
			for ( uint32 cls_num : _clusters_number )
			{
				...
				while ( *( char * ) desc_ptr )
				{
					...
					{
						printfYellow( "[Fat32Inode] read sub-dir fail. sub-dir name = %s", dirname.c_str() );
						k_bufm.release_buffer_sync( buf );
						return nullptr;
					}
				...
				if ( find )
				{
					...
					{
						printfYellow( "[Fat32Inode] no mem to create new inode." );
						k_bufm.release_buffer_sync( buf );
						return nullptr;
					}
					break;
...
// ================ private helper function ================
		...
		int Fat32Inode::_read_sub_dir( void * &sub_desc, eastl::string &out_dir_name, Fat32DirectryShort &out_dir_info )
		{
			...
			else // save the long name as dir-name
			{
				...
				{
					printfRed( "Fat32DirEntry: dir-entry ptr is not pointing at the last entry." );
					return -2;
				}
				...
				for ( ; dirlong >= ( Fat32DirectryLong * ) sub_desc; dirlong-- )
				{
					...
					{
						printfRed( "Fat32DirEntry: read bad file name." );
						return -1;
					}
