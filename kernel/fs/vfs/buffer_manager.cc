...
namespace fs
{
	...
	int BufferManager::dirt_buffer(Buffer &buf)
	{
		uint blk = buf._block_number;
		uint idx = buf._buffer_index;
		if (blk >= block_per_pool)
			return -1;
		if (idx >= _buffer_pool[blk]._current_buffer_counts)
			return -2;
	...
	void BufferManager::_release_buffer(Buffer &buf, bool used_sleep_lock)
	{
		...
		if (used_sleep_lock &&
			...
			panic("not hold buffer sleep-lock");
		if (used_sleep_lock)
	...
	{
		dev::VirtualDevice *dev = dev::k_devm.get_device(dev_num);
		if (dev == nullptr)
		{
			printfYellow("No.%d device is (null)", dev_num);
			return -1;
		}
		if (dev->type() != dev::dev_block)
		{
			printfYellow("No.%d device is not a block device", dev_num);
			return -2;
		}
		return 0;
	}
