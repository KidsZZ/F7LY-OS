...
enum PteEnum
{
    ...
    _build_pte_bit_(readable, 0x1, 1)    // R: 可读位
    _build_pte_bit_(writable, 0x1, 2)    // W: 可写位
    _build_pte_bit_(executable, 0x1, 3)  // X: 可执行位
    _build_pte_bit_(user, 0x1, 4)        // U: 用户可访问位
    _build_pte_bit_(global, 0x1, 5)      // G: 全局映射位
    _build_pte_bit_(accessed, 0x1, 6)    // A: 访问位
    _build_pte_bit_(dirty, 0x1, 7)       // D: 脏位
    _build_pte_bit_(rsw, 0x3, 8)         // RSW: 保留给软件使用的位
